-- MystiCartes V2: pondération des récompenses par rareté et difficulté.
-- Les fonctions auxiliaires rendent les règles déterministes et testables.

create or replace function public.solo_match_reward_multiplier_v2(
  p_difficulty text
)
returns numeric
language sql
...
create or replace function public.solo_match_xp_reward_v2(
  p_result text,
  p_difficulty text
)
returns integer
...
as $$
  select ceil(
    ...
    end) * public.solo_match_reward_multiplier_v2(p_difficulty)
  )::integer;
$$;

create or replace function public.solo_match_gold_reward_v2(
  p_result text,
  p_difficulty text
)
returns integer
...
create or replace function public.solo_match_reward_rarity_v2(
...
as $$
  select case
    when p_roll >= 0.00 and p_roll < 0.70 then 'commune'
    when p_roll >= 0.70 and p_roll < 0.90 then 'rare'
    when p_roll >= 0.90 and p_roll < 0.98 then 'épique'
    when p_roll >= 0.98 and p_roll < 1.00 then 'légendaire'
    else null
  end;
$$;

revoke all on function public.solo_match_reward_multiplier_v2(text)
  from public, anon, authenticated;
revoke all on function public.solo_match_xp_reward_v2(text, text)
  from public, anon, authenticated;
revoke all on function public.solo_match_gold_reward_v2(text, text)
  from public, anon, authenticated;
revoke all on function public.solo_match_reward_rarity_v2(double precision)
  from public, anon, authenticated;
...
begin
  ...
  v_xp := public.solo_match_xp_reward_v2(p_result, v_difficulty);
  v_gold := public.solo_match_gold_reward_v2(p_result, v_difficulty);
  ...
  if p_result = 'win' then
    v_reward_rarity := public.solo_match_reward_rarity_v2(random());

    select c.id
      into v_reward_card_id
      from public.cards c
      where c.is_active
        and c.rarity = v_reward_rarity
        and (
          c.set_id is null
          or exists (
    ...
    if v_reward_card_id is null then
      select c.id
        into v_reward_card_id
        from public.cards c
        where c.is_active
